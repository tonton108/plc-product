...
詳細は `_docs/plc-knowledge/protocols.md` を参照。
...
詳細は `_docs/features/local-buffering.md` を参照（未作成）。
...
詳細は `_docs/deployment/environment-variables.md` を参照。

## 関連ドキュメント

- `_docs/plc-knowledge/protocols.md` - PLCプロトコル実装ガイド
- `_docs/plc-knowledge/troubleshooting.md` - トラブルシューティング
- `_docs/deployment/raspi-deployment.md` - ラズパイデプロイ手順
