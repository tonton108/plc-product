...
## 負荷検証結果（2026-07-10、`scripts/load_test_200.py`）

200設備を登録し、同時POST（32並列）で5000ログを投入した結果（compose開発サーバ）:
- 登録 200/200 成功、ログPOST 5000/5000 成功（**エラー0・デッドロック無し**）
- 全ログが当月パーティション `logs_2026_07` へ正しくルーティング（動的項目`sensor_a`も保存）
- 読み取りレイテンシ latest p50≈8ms / history(100) p50≈9ms
- **大量データのクリーンアップ**: 旧月2万件＋境界月混在8千件を投入し `cleanup_old_logs(30)` を実行 → 旧月パーティションをDROP（2万件を瞬時）＋境界5千件をDELETE、**計25000行の削除が0.14秒**。境界月の保持対象・当月分は正しく残存

注: `flask run`（開発サーバ）のため絶対スループット（約224 req/s）は本番Waitress値ではない。本番構成の負荷検証はPhase 4の別ゲート。
