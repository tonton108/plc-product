...
def test_chart():
    with sync_playwright() as p:
        try:
            ...
            # デバッグボタンをクリック
            print("[INFO] Clicking debug button...")
            try:
                page.click('.v-fab button', timeout=5000)  # FABボタンをクリック
            except:
                try:
                    page.click('button.v-btn', timeout=5000)  # フォールバック
                except:
                    print("[WARNING] Could not click debug button, continuing anyway...")
            time.sleep(2)
            ...
            # スクリーンショット（デバッグ情報が表示されている状態）
            page.screenshot(path='debug_with_debugmode.png', full_page=True)
            print("[INFO] Screenshot saved: debug_with_debugmode.png")
            ...
            if chart_state['canvasElements'] >= 5:
                ...
                print(f"[INFO] Check screenshot for debug information")
            else:
