...
def simulate_interval_impact(interval_ms, server_timeout_sec=5):
    ...
    print(f"  総タイムアウト待ち時間: {total_timeout_time:,}秒 ({total_timeout_time/60:.1f}分)")
    ...
    print(f"  1日ダウン時のバッファデータ量: {buffer_size_mb:.2f}MB ({buffer_records_per_day:,}件)")
    ...
    if effective_interval > interval_ms * 2:
        warnings.append("[WARNING] サーバーダウン時に実質的なデータ取得頻度が半分以下になる")

    if sends_per_day > 100000:
        warnings.append("[WARNING] 1日10万回以上の送信：サーバー負荷が高い")

    if buffer_records_per_day > 50000:
        warnings.append("[WARNING] 1日ダウン時のバッファ件数が5万件超：再送に時間がかかる")
...
def main():
    ...
    print("  送信間隔別の比較表")
    print("=" * 70)
    print(f"{'送信間隔':<12} {'1日の送信回数':<15} {'ダウン時の実質間隔':<20} {'1日ダウン時のバッファ件数':<25} {'評価'}")
    print("-" * 70)
    ...
    for result in results:
        ...
        print(f"{interval:>6}ms     {sends:>12,}回     {effective:>10.0f}ms          {buffer:>15,}件        {status}")
