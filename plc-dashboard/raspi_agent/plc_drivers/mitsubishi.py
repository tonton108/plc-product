        ...
        try:
            if group["count"] == 1:
                ...
                raw_values = plc.batchread_wordunits(
                    headdevice=f"D{group['start_address']}", readsize=1
                )
            else:
                # 連続アドレス → バッチ読み取り（最適化）
                logger.info(
                    f"🚀 バッチ読み取り: D{group['start_address']}-D{group['start_address'] + group['count'] - 1} ({group['count']}ワード)"
                )
                raw_values = plc.batchread_wordunits(
                    headdevice=f"D{group['start_address']}", readsize=group["count"]
                )
    ...
    for key, setting in data_points.items():
        if setting.get("enabled", False):
            ...
            if address:
                    ...
                    if data_type == "bit":
                        ...
                        if "." in address:
                            ...
                            bit_values = plc.batchread_bitunits(
                                headdevice=f"{device_type}{addr_num}", readsize=1
                            )
                        ...
                        raw_value = plc.batchread_wordunits(
                            headdevice=f"D{addr_num}", readsize=1
                        )[
                            0
                        ]  # リストの最初の要素を取得
                ...
                # 安全な読み取り実行
                raw_value = safe_plc_read(
                    read_mitsubishi_data, f"{key}({address})読み取り"
                )
